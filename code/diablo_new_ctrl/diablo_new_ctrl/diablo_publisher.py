...
class KeyboardControlNode(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info(
            "Contrôle du robot Diablo : Flèches (déplacement), + / - (vitesse), m (marche), j (saut), s (split), espace (stop), Ctrl+C (quitter)"
        )

        # Paramètres de vitesse
        self.speed = 1.0  # Vitesse linéaire initiale
        self.turn_speed = 0.5  # Vitesse de rotation initiale
        self.speed_increment = 0.1  # Incrément de vitesse lors des ajustements

        # États des modes (activés/désactivés)
    ...
    def run_keyboard_control(self):
        ...
        try:
            while rclpy.ok():  # Boucle active tant que ROS 2 fonctionne
                ...
                elif key == 'm':  # Activation/Désactivation du mode marche
                    self.stand_mode = not self.stand_mode  # Inversion de l'état actuel
                    msg.mode.stand_mode = self.stand_mode
                    self.get_logger().info(f"Mode marche {'activé' if self.stand_mode else 'désactivé'}")

                elif key == 'j':  # Activation/Désactivation du mode saut
                    self.jump_mode = not self.jump_mode
                    msg.mode.jump_mode = self.jump_mode
                    self.get_logger().info(f"Mode saut {'activé' if self.jump_mode else 'désactivé'}")

                elif key == 's':  # Activation/Désactivation du mode split
                    self.split_mode = not self.split_mode
                    msg.mode.split_mode = self.split_mode
                    self.get_logger().info(f"Mode split {'activé' if self.split_mode else 'désactivé'}")
